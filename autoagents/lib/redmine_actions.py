...
def compute_task_duration(task_path: Path, summary_block: str) -> str | None:
    """
    Return a human duration label from filename stamp → Closed at.
    Returns None when duration cannot be computed.
    """
...
def post_task_completion_note(task_path: Path, issue_id: int | None = None) -> str:
    ...
    Includes task duration in the note text only (does not post a time_entry).
